...
{
	int err = acs_key_desc_build_response(buf, &reply->response->b, reply->conn);
...
{
	int err = acs_isc_build_response(buf, &reply->response->b);
...
{
	int err = acs_rhandle_build_map_response(&reply->response->b);
...
int acs_cp_handle_get_svc_char_uuids(struct acs_reply *reply, struct net_buf_simple *buf)
{
	...
	err = acs_rhandle_lookup_svc_char(resource_handle, &reply->response->b);
