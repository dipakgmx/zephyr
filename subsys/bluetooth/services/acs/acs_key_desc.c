...
int acs_key_desc_validate_records(void)
{
	...
	STRUCT_SECTION_FOREACH(bt_acs_key_desc_record, rec) {
		...
		/* The nonce prefix must fit the per-record runtime buffer. */
		if (acs_key_desc_has_nonce_record(rec) &&
		    acs_key_desc_nonce_fixed_size(rec) > ACS_MAX_NONCE_PREFIX_SIZE) {
			LOG_ERR("key descriptor 0x%04x nonce fixed size %u exceeds %u", rec->key_id,
				acs_key_desc_nonce_fixed_size(rec), ACS_MAX_NONCE_PREFIX_SIZE);
			return -EINVAL;
		}
	...
	/* Every descriptor record needs a runtime slot. */
	if (algo_records > ACS_ALGO_RECORD_COUNT) {
		LOG_ERR("%zu algorithm key descriptor records exceed %u runtime slots",
			algo_records, ACS_ALGO_RECORD_COUNT);
		return -ENOSPC;
	}
...
/* Serialize an AES algorithm record, omitting nonce fields for CMAC (Table 4.45). */
static int append_aes_record(const struct bt_acs_key_desc_record *rec, struct net_buf *buf,
			     struct bt_acs_conn *acs_conn)
{
	if (rec->type_id == ACS_KEY_REC_AES_128_CMAC) {
		const uint8_t total =
			sizeof(struct acs_desc_rec_hdr) + ACS_KEY_DESC_AES_CMAC_DATA_SIZE;
	...
	net_buf_add_mem(buf, &wire, sizeof(wire));
	...
	if (rec->aes.nonce_fixed_size > 0) {
		...
		net_buf_add_mem(buf, server_fixed_nonce, rec->aes.nonce_fixed_size);
	}
				...
				struct bt_acs_conn *acs_conn)
{
	...
	STRUCT_SECTION_FOREACH(bt_acs_key_desc_record, rec) {
		...
		if (rec->type_id == ACS_KEY_REC_ECDH) {
			...
			if (net_buf_tailroom(buf) < sizeof(wire)) {
				return -ENOMEM;
			}
			net_buf_add_mem(buf, &wire, sizeof(wire));
		...
		} else if (acs_key_desc_is_algorithm_record(rec)) {
...
{
	int err = acs_key_desc_build_response(buf, reply->response, reply->conn);
