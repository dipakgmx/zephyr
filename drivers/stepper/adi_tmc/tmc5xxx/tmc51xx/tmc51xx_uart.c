				  ...
				  const uint32_t reg_val)
{
	...
	err = tmc_uart_write_register(config->bus.uart, config->uart_slave_addr, reg_addr, reg_val);
				 ...
				 uint32_t *reg_val)
{
	...
	/* Route to the adi_tmc_uart.h implementation */
	err = tmc_uart_read_register(config->bus.uart, config->uart_slave_addr, reg_addr, reg_val);
