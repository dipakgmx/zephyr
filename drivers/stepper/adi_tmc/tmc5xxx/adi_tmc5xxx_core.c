/**
 * @file drivers/stepper/adi_tmc/tmc5xxx/adi_tmc5xxx_core.c
 * @brief Common core implementation for TMC5xxx stepper controllers
 */
...
int tmc5xxx_move_to(const struct device *dev, int32_t position)
{
	...
	/* First set the ramp mode to positioning mode */
	err = tmc5xxx_write_reg(ctx, TMC5XXX_RAMPMODE(ctx->motor_index),
	...
	return tmc5xxx_write_reg(ctx, TMC5XXX_XTARGET(ctx->motor_index), (uint32_t)position);
}
...
int tmc5xxx_get_micro_step_res(const struct device *dev, enum stepper_micro_step_resolution *res)
{
	...
	*res = (reg_value & TMC5XXX_CHOPCONF_MRES_MASK) >> TMC5XXX_CHOPCONF_MRES_SHIFT;
...
int tmc5xxx_stallguard_enable(const struct device *dev, bool enable)
{
	...
	if (enable) {
		...
		if (abs(actual_velocity) < config->sg_threshold_velocity) {
			LOG_ERR("%s: StallGuard not enabled, actual velocity below threshold", ctx->dev->name);
			return -EAGAIN;
		}
	} else {
		reg_value &= ~TMC5XXX_SW_MODE_SG_STOP_ENABLE;
...
void tmc5xxx_rampstat_work_handler(struct k_work *work)
{
	...
	struct tmc5xxx_stepper_data *data =
	...
	/* Extract the interrupt bits */
	uint8_t int_flags = (rampstat & TMC5XXX_RAMPSTAT_INT_MASK) >> TMC5XXX_RAMPSTAT_INT_SHIFT;
			       ...
			       void *user_data)
{
	...
	LOG_DBG("%s: Event callback set", data->core.dev->name);
	return 0;
}
		     ...
		     const struct tmc_ramp_generator_data *ramp_data)
{
	...
	LOG_DBG("%s: Configuring ramp generator", ctx->dev->name);

	/* Set acceleration and velocity values */
	err = tmc5xxx_write_reg(ctx, ctx->reg_map->vstart, ramp_data->vstart);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->a1, ramp_data->a1);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->v1, ramp_data->v1);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->amax, ramp_data->amax);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->vmax, ramp_data->vmax);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->dmax, ramp_data->dmax);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->d1, ramp_data->d1);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->vstop, ramp_data->vstop);
	if (err != 0) {
		return err;
	}

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->tzerowait, ramp_data->tzerowait);
	if (err != 0) {
		return err;
	}

	/* Set additional timing parameters if available */
	if (ctx->reg_map->vcoolthrs != 0) {
		err = tmc5xxx_write_reg(ctx, ctx->reg_map->vcoolthrs, ramp_data->tcoolthrs);
		if (err != 0) {
			return err;
		}
	}

	if (ctx->reg_map->vhigh != 0) {
		err = tmc5xxx_write_reg(ctx, ctx->reg_map->vhigh, ramp_data->thigh);
		if (err != 0) {
			return err;
		}
	}
	...
	uint32_t iholdrun = TMC5XXX_IHOLD(ramp_data->ihold) | TMC5XXX_IRUN(ramp_data->irun) |
			    TMC5XXX_IHOLDDELAY(ramp_data->iholddelay);

	err = tmc5xxx_write_reg(ctx, ctx->reg_map->ihold_irun, iholdrun);
	if (err != 0) {
		return err;
	}

	LOG_DBG("%s: Ramp generator configured successfully", ctx->dev->name);
	return 0;
}
#endif /* CONFIG_STEPPER_ADI_TMC_RAMP_GEN */
